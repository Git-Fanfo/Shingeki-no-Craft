...
#Throw
execute as @s[tag=!wrap,scores={odm_state=0,man_cool=0},predicate=snc:has_gas_trigger,predicate=snc:has_odm] run function snc:player/odm/throw
...
# Build
execute as @s[predicate=snc:build/has_build] run function snc:player/build/select_build
# Rename
execute if predicate snc:villager/has_rename run function snc:player/build/bell/rename/main

# Shoot
execute as @s[predicate=snc:gun/has_gun] anchored eyes positioned ^ ^ ^ run function snc:player/gun/select_gun