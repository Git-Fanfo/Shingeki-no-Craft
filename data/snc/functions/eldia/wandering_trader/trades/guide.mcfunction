## Book 1
# function snc:give/guide/shipload

data merge entity @s {Offers:{Recipes:[{buy:{id:"minecraft:emerald",Count:1b},sell:{id:"minecraft:written_book",Count:1b,tag:{CustomModelData:1,generation:2,title:"Cart's Shipload",author:"Marvin Mallmeister",pages:["{\"text\":\"\\uE311\",\"color\":\"white\"}",\
"[{\"text\":\"\\uE301\",\"color\":\"white\"},\
{\"text\":\"1. \",\"color\":\"dark_red\",\"bold\":true},{\"text\":\"Barrels\",\"color\":\"black\",\"clickEvent\":{\"action\":\"change_page\",\"value\":\"3\"}},\"\\n\",\
{\"text\":\"2. \",\"color\":\"dark_red\",\"bold\":true},{\"text\":\"Turrets\",\"color\":\"black\",\"clickEvent\":{\"action\":\"change_page\",\"value\":\"9\"}},\"\\n\",\
{\"text\":\"3. \",\"color\":\"dark_red\",\"bold\":true},{\"text\":\"Cannon\",\"color\":\"black\",\"clickEvent\":{\"action\":\"change_page\",\"value\":\"9\"}}]",\
"[{\"text\":\"\\uE312\",\"color\":\"white\"},{\"text\":\"\\n\\n\\n\\n\\n\\n\\n\\n\\n\\n\\n\\n\\n \\uE300\",\"bold\":true,\"clickEvent\":{\"action\":\"change_page\",\"value\":\"2\"}}]",\
"[{\"text\":\"\\uE313\",\"color\":\"white\"},{\"text\":\"\\n\\n\\n\\n\\n\\n\\n\\n\\n\\n\\n\\n\\n \\uE300\",\"bold\":true,\"clickEvent\":{\"action\":\"change_page\",\"value\":\"2\"}}]",\
"[{\"text\":\"\\uE314\",\"color\":\"white\"},{\"text\":\"\\n\\n\\n\\n\\n\\n\\n\\n\\n\\n\\n\\n\\n \\uE300\",\"bold\":true,\"clickEvent\":{\"action\":\"change_page\",\"value\":\"2\"}}]",\
"[{\"text\":\"\\uE315\",\"color\":\"white\"},{\"text\":\"\\n\\n\\n\\n\\n\\n\\n\\n\\n\\n\\n\\n\\n \\uE300\",\"bold\":true,\"clickEvent\":{\"action\":\"change_page\",\"value\":\"2\"}}]",\
"[{\"text\":\"\\uE302\",\"color\":\"white\"},{\"text\":\"\\n\\n\\n\\n\\n\\n\\n\\n\\n\\n\\n\\n\\n \\uE300\",\"bold\":true,\"clickEvent\":{\"action\":\"change_page\",\"value\":\"2\"}}]",\
"[{\"text\":\"\\uE303\",\"color\":\"white\"},{\"text\":\"\\n\\n\\n\\n\\n\\n\\n\\n\\n\\n\\n\\n\\n \\uE300\",\"bold\":true,\"clickEvent\":{\"action\":\"change_page\",\"value\":\"2\"}}]",\
"[{\"text\":\"\\uE316\",\"color\":\"white\"},{\"text\":\"\\n\\n\\n\\n\\n\\n\\n\\n\\n\\n\\n\\n\\n \\uE300\",\"bold\":true,\"clickEvent\":{\"action\":\"change_page\",\"value\":\"2\"}}]",\
"[{\"text\":\"\\uE317\",\"color\":\"white\"},{\"text\":\"\\n\\n\\n\\n\\n\\n\\n\\n\\n\\n\\n\\n\\n \\uE300\",\"bold\":true,\"clickEvent\":{\"action\":\"change_page\",\"value\":\"2\"}}]",\
"[{\"text\":\"\\uE318\",\"color\":\"white\"},{\"text\":\"\\n\\n\\n\\n\\n\\n\\n\\n\\n\\n\\n\\n\\n \\uE300\",\"bold\":true,\"clickEvent\":{\"action\":\"change_page\",\"value\":\"2\"}}]",\
"[{\"text\":\"\\uE319\",\"color\":\"white\"},{\"text\":\"\\n\\n\\n\\n\\n\\n\\n\\n\\n\\n\\n\\n\\n \\uE300\",\"bold\":true,\"clickEvent\":{\"action\":\"change_page\",\"value\":\"2\"}}]",\
"[{\"text\":\"\\uE302\",\"color\":\"white\"},{\"text\":\"\\n\\n\\n\\n\\n\\n\\n\\n\\n\\n\\n\\n\\n \\uE300\",\"bold\":true,\"clickEvent\":{\"action\":\"change_page\",\"value\":\"2\"}}]",\
"[{\"text\":\"\\uE303\",\"color\":\"white\"},{\"text\":\"\\n\\n\\n\\n\\n\\n\\n\\n\\n\\n\\n\\n\\n \\uE300\",\"bold\":true,\"clickEvent\":{\"action\":\"change_page\",\"value\":\"2\"}}]"]}}},\
{buy:{id:"minecraft:emerald",Count:1b},sell:{id:"minecraft:apple",Count:1b}}\
]}}