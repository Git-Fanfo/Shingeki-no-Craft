...
$execute if score $destroy_when_spawn $(shifter)_vars matches 1 run advancement grant @s only snc:shifters/nuke
$execute if score $destroy_when_spawn $(shifter)_vars matches 1 run summon creeper ~ ~4 ~ {ExplosionRadius:10b,Fuse:0,CustomName:'{"text":"$(name)"}'}
