# Tp player to neck
execute as @p[scores={beast_vars=1}] rotated ~ 0 run tp ^ ^11 ^-1

# Start riding armor_stand instead of horse
execute on vehicle run kill
ride @s dismount

summon armor_stand ^ ^ ^ {Invisible:1b,Tags:["beast","dead","shifter"],Pose:{Head:[20f,0f,0f]},ArmorItems:[{},{},{},{id:"minecraft:carved_pumpkin",Count:1b,components:{"custom_model_data":58}}]}
