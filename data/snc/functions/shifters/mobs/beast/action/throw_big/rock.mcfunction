...
execute if score $shifter_destroy config matches 1 on vehicle \
...
## Crater
execute if score $shifter_destroy config matches 1 on passengers if entity @s[type=item_display,tag=beast_wrap] on vehicle unless predicate snc:is_riding align xyz positioned ~.5 ~1 ~.5 run function snc:shifters/mobs/beast/action/throw_big/summon
