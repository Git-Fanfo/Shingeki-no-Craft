...
$execute if entity @s[tag=aj.$(shifter).locator.consume] if score $hold $(shifter)_vars matches 1 unless data entity @s Passengers run ride @e[type=#snc:eldian,tag=snc.handcuffs,distance=..5,sort=nearest,limit=1] dismount
