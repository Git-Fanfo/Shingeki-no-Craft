...
$execute if entity @s[tag=aj.$(shifter).locator.consume] if score $hold $(shifter)_vars matches 1 unless data entity @s Passengers run ride @n[type=#snc:eldian,tag=snc.handcuffs,distance=..5] dismount
$execute if entity @s[tag=aj.$(shifter).locator.consume] if score $hold $(shifter)_vars matches 1 unless data entity @s Passengers run ride @n[type=#snc:eldian,tag=!transform,distance=..5] mount @s
