...
## Destroy?
#$execute if score $shifter_destroy config matches 1 positioned over world_surface run fill ^$(half_minus_zero_point_five) ^$(half_minus_zero_point_five) ^$(half_minus_zero_point_five) ^-$(half_minus_zero_point_five) ^ ^-$(half_minus_zero_point_five) air destroy
## Play sound
...
$execute as @e[distance=..$(distance),tag=transform,type=!#snc:pivot,predicate=!snc:shifters/$(shifter)/score] on vehicle at @s rotated as @n[tag=$(shifter)] rotated ~ -$(angle) run function snc:logic/motion/generic {"score":"$(shifter)_vars","strength":0.0$(knockback), "unstoppable":"true"}
# Set direction
