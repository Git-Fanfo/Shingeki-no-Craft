...
$execute positioned over world_surface run fill ^$(half_minus_zero_point_five) ^$(half_minus_zero_point_five) ^$(half_minus_zero_point_five) ^-$(half_minus_zero_point_five) ^ ^-$(half_minus_zero_point_five) air destroy
## Play sound
...
$execute as @e[distance=..$(distance),tag=transform,type=!#snc:pivot,predicate=!snc:shifters/$(shifter)/score] on vehicle rotated ~ -$(knockback) run function snc:logic/motion/generic {"score":"$(shifter)_vars","strength":0.02}
#$execute as @e[distance=..$(distance),tag=transform,type=!#snc:pivot,predicate=!snc:shifters/$(shifter)/score] on vehicle at @s rotated ~ -$(knockback) run function snc:logic/motion/generic {"score":"$(shifter)_vars","strength":0.01}
