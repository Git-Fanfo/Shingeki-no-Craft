...
# ID doesn't match
execute unless score @s snc.lock matches 1 unless score @s snc.key = @n[tag=snc.handcuffs,type=#snc:eldian] snc.lock run scoreboard players set @s snc.lock 2
...
execute if score @s snc.lock matches 2 run tellraw @s ["",{"text":"🔒 ","color":"red"},{"translate":"aot.key.no.desc3","color":"yellow"}," ",{"translate":"aot.key.no.desc4"},{"text":" #","color":"yellow","italic": true},{"score":{"name":"@n[tag=snc.handcuffs,type=#snc:eldian]","objective":"snc.lock"},"color":"yellow","italic": true}]
...
execute if score @s snc.lock matches 0 as @n[tag=snc.handcuffs.vehicle] run function snc:interact/handcuffs/kill
